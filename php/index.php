...
function rgbToXYZ($r, $g, $b, $dec = 2) {
    ...
    $G = linearize($g);
    $B = linearize($b);
    $X = $R * 0.412453 + $G * 0.357579 + $B * 0.180438;
    $Y = $R * 0.212671 + $G * 0.715152 + $B * 0.072177;
    $Z = $R * 0.019334 + $G * 0.119193 + $B * 0.950304;
    $X = round($X, $dec);
    $Y = round($Y, $dec);
    $Z = round($Z, $dec);
    return array($X, $Y, $Z);
...
function rgbToLch($r, $g, $b, $dec = 0) {
    list ($l, $a, $b) = rgbToLab($r, $g, $b, 8);
    $c = sqrt($a*$a + $b*$b);
    $h = atan2($b, $a);
    $h = rad2deg($h);
    if ($h < 0)
